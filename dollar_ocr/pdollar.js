 ...
 *
 * A variation of the $1 recognizer that treats gestures as unordered point clouds.
 * This makes it invariant to stroke direction and starting point.
 *
 * Adapted for ES6 Modules and [x,y] array input.
 */

export default class PDollar {
    constructor() {
        this.NumPoints = 32; // $P typically uses fewer points than $1 (32 is standard)
        this.Origin = { x: 0, y: 0 };
    ...
    /**
     * Primary method to recognize a gesture using the $P algorithm.
     * @param {Array} points - An array of points [[x,y], [x,y]...]
    ...
    check(points) {
        ...
        for (let i = 0; i < this.Templates.length; i++) {
            const d = this._greedyCloudMatch(processedPoints, this.Templates[i].Points);
            if (d < minDistance) {
                minDistance = d;
        ...
        if (bestTemplate) {
            score = Math.max((minDistance - 2.0) / -2.0, 0.0);
        }
    ...
    _convertPoints(rawPoints) {
        // FIX: Assign ID: 1 to all points.
        // This ensures the resampler treats the array as a single continuous stroke
        // rather than a series of disconnected dots.
        if (Array.isArray(rawPoints[0])) {
            return rawPoints.map(p => ({x: p[0], y: p[1], id: 1})); 
        }
        return rawPoints;
    }
    ...
    _resample(points, n) {
        ...
        for (let i = 1; i < points.length; i++) {
            if (points[i].id === points[i-1].id) { // Ensure same stroke if handling multi-stroke
    ...
    _pathLength(points) {
        let d = 0.0;
        for (let i = 1; i < points.length; i++) {
            if (points[i].id === points[i-1].id) // Check stroke ID if multistroke
    ...
    // $P uses a greedy matching algorithm rather than rotation checking
    _greedyCloudMatch(points, template) {
        const e = 0.50;
        const step = Math.floor(Math.pow(points.length, 1.0 - e));
        let min = +Infinity;
        for (let i = 0; i < points.length; i += step) {
            const d1 = this._cloudDistance(points, template, i);
            const d2 = this._cloudDistance(template, points, i);
            min = Math.min(min, Math.min(d1, d2));
        }
        return min;
    }

    _cloudDistance(pts1, pts2, start) {
        // pts2 is template
        ...
        do {
            ...
            for (let j = 0; j < matched.length; j++) {
                if (!matched[j]) {
                    const d = this._distance(pts1[i], pts2[j]);
                    if (d < min) {
                        min = d;
                        index = j;
                    }
